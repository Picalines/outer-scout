...
internal sealed class WarpRouteDefinition : IApiRouteDefinition
{
    ...
    public void MapRoutes(HttpServerBuilder serverBuilder, IApiRouteDefinition.IContext context)
    {
        ...
        serverBuilder.Map(HttpMethod.Post, "warp_to/:ground_body_name", (Request request, string ground_body_name) =>
        {
            ...
            {
                return ResponseFabric.BadRequest($"ground body \"{ground_body_name}\" not found");
            }
            ...
            var spawnPoint = groundBodyTransform.Find(ModSpawnPointName).OrNull()
                ?.GetComponent<SpawnPoint>();

            if (spawnPoint is null)
            {
                var referenceSpawnPoint = groundBodyTransform.GetComponentsInChildren<SpawnPoint>()
                    .Select(point => new { point, distanceToPlayer = (point.transform.position - localTransform.position).magnitude })
                ...
                {
                    newSpawnPointGameObject = UnityEngine.Object.Instantiate(referenceSpawnPoint.gameObject);
                }
